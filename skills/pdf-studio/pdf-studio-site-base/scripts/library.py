...
is shared across document studios, e.g. pdf-studio and paper-studio), so the index
...
to "pdf-studio" so an existing library keeps working with no migration.

Layout:
  <root>/                       $XDG_DATA_HOME/<namespace> (default pdf-studio; or ~/.local/share/...)
...
# --namespace or the LIBRARY_NAMESPACE env var; defaults to "pdf-studio" so an
# existing ~/.local/share/pdf-studio/ library keeps working with no migration.
# main() overwrites this from the parsed --namespace before dispatching a command.
LIBRARY_NAMESPACE = os.environ.get("LIBRARY_NAMESPACE") or "pdf-studio"
...
# across studios (pdf-studio, paper-studio), so the generator/footer name carries
...
def main():
    ...
    # LIBRARY_NAMESPACE env var; both default to "pdf-studio" (no migration).
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument(
        "--namespace",
        default=os.environ.get("LIBRARY_NAMESPACE") or "pdf-studio",
        help="XDG library namespace under $XDG_DATA_HOME (default: pdf-studio)",
    )
