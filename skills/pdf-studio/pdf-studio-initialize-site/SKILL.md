...
- Not already initialized: `python3 pdf-studio-site-base/scripts/library.py status --namespace pdf-studio`. If it prints existing metadata, the library exists — skip this skill and use [[pdf-studio-deploy-site]].
...
2. **Create the local library:**
   ...
   python3 pdf-studio-site-base/scripts/library.py \
     init --namespace pdf-studio --project <project> --title "<display title>"
   ```
...
3. **Create the Pages project:** `wrangler pages project create <project> --production-branch=main`. If wrangler reports the name is taken, the deployed URL would get a random suffix — pick another name and redo steps 1–3 (re-run `library.py init --namespace pdf-studio --force --project <new>`).
...
5. **First deploy (empty index) and verify:**
   ```sh
   wrangler pages deploy "$(python3 pdf-studio-site-base/scripts/library.py public --namespace pdf-studio)" \
