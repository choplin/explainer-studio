...
table, an unknown callout variant). Every consumer takes this route (pdf-explainer,
paper-explainer, explain-diff).
...
- **Inline mode** (a single self-contained file): `base.css`/`base.js` are folded into
  ...
  [[diff-explainer]] takes this route.)
...
or [[diff-explainer]]'s risk / verify axes) supplies:
...
The **pdf-explainer** consumer adds these (their markup carries a raw `<audio>` / must
...
it; a multi-page site (pdf-explainer) stays copy-mode so pages share one asset set.
...
scripts/build-site.sh <src-dir> <out-dir> \
  --assets <html-docs/assets> [--context <dir>] [--component <name>]...
```
...
- **The semantic half** — is this the right variant, is this actually the takeaway,
  ...
  reading site (pdf-explainer / paper-explainer deliberately skip it as YAGNI) or
  **high-stakes but consumer-specific**, in which case the consuming skill
  **internalizes it as its own completion check** — e.g. [[diff-explainer]]
...
`html-docs` review step to run. A consumer that layers high-stakes
...
filter="…"}`, or set by hand on a consumer's own index markup (e.g. the pdf-explainer
...
reading page and on an [[diff-explainer]] page.
...
- **`base.js` targets the first `main article`.** A page that puts several
  ...
  [[diff-explainer]] takes that route (its own template variant). This is
...
- **Nothing here fails loudly at the *semantic* level.** The generator refuses a
  ...
  [[diff-explainer]] does for `risk`/`tested`/`verified`).
