...
| `reading-site-library-base` | Shared resources, delegated to by name (not invoked directly): the Cloudflare Pages library manager (`library.py`) and the pdf-explainer content context layer (`pdf-explainer.css`), layered on the `html-docs` base design system. The reading-site nav widgets are `html-docs`' `reading-nav` opt-in component (pulled in via `--component reading-nav`). |
