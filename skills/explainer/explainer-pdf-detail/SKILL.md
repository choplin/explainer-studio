---
name: explainer-pdf-detail
description: Internal procedure for the explainer-deep-dive skill — re-read a resolved page span of an already-digested PDF (visually, or from a faithful text layer) and write a thorough, standalone detail report for just that span, with [pNN] anchors. Applied by the deep-dive orchestrator (dispatched to a explainer-pdf-detail subagent under Claude Code, or applied inline otherwise), and by pdf-explainer-full-guide once per in-scope chapter. NOT a user-facing skill and NOT triggered directly by user requests.
...
The `explainer-deep-dive` skill resolves the target page range from the outline's [pNN] anchors and then applies this procedure; `pdf-explainer-full-guide` applies it once per in-scope chapter. It is not for direct user requests and is not invoked proactively.
...
Write to the given output path — unconditionally, without prompting about an existing file. This is an orchestrator-dispatched worker; the parent skill ([[explainer-deep-dive]] / [[pdf-explainer-full-guide]]) handles overwrite confirmation before dispatching.
