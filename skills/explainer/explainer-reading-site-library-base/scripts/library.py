#!/usr/bin/env python3
"""Manage a reading-library site under $XDG_DATA_HOME/<namespace>.

The library is a single, persistent deploy root that accumulates documents as
subpaths, so one Cloudflare Pages project (and one Access policy) serves the
whole collection. It renders a document-type-neutral index (the same deploy root
is shared across document studios, e.g. pdf-explainer and paper-explainer), so the index
carries no per-studio vocabulary or identity. This script does only local
filesystem + JSON work — no network, no wrangler. The skills run wrangler
separately.

The XDG namespace is caller-injected (--namespace / LIBRARY_NAMESPACE), defaulting
to "pdf-explainer" so an existing library keeps working with no migration.

Layout:
  <root>/                       $XDG_DATA_HOME/<namespace> (default pdf-explainer; or ~/.local/share/...)
  ...
  └── public/                   the deploy root (uploaded whole by wrangler)
      ...
      ├── assets/base.css       shared base design system (from explainer-html-docs)
      ├── assets/reading-site.css pdf-explainer content layer (from explainer-reading-site-library-base)
      ├── assets/reading-nav.css reading-nav widget chrome (from explainer-html-docs component)
      ├── assets/base.js        base PE kit: theme toggle, back-to-top (from explainer-html-docs)
      ├── assets/reading-nav.js card live filter + page nav (from explainer-html-docs component)
...
# The XDG library namespace (the subdir under $XDG_DATA_HOME). Caller-injected via
# --namespace or the LIBRARY_NAMESPACE env var; defaults to "pdf-explainer" so an
# existing ~/.local/share/pdf-explainer/ library keeps working with no migration.
# main() overwrites this from the parsed --namespace before dispatching a command.
LIBRARY_NAMESPACE = os.environ.get("LIBRARY_NAMESPACE") or "pdf-explainer"
...
def root():
    ...
    return Path(base) / LIBRARY_NAMESPACE
...
def base_css_source():
    # The shared base design system, owned by the sibling explainer-html-docs
    # skill (skills install as flat siblings under the skills root):
    # <root>/explainer-reading-site-library-base/scripts/library.py
    #   -> <root>/explainer-html-docs/assets/base.css
    return (
...
def context_css_source():
    ...
    # explainer-reading-site-library-base/scripts/library.py -> explainer-reading-site-library-base/assets/reading-site.css
...
def base_js_source():
    ...
    # owned by the sibling explainer-html-docs skill:
    # <root>/explainer-reading-site-library-base/scripts/library.py
    #   -> <root>/explainer-html-docs/assets/base.js
    return (
        Path(__file__).resolve().parents[2]
        / "explainer-html-docs"
        / "assets"
...
def reading_nav_css_source():
    # The reading-nav widget chrome, owned by the sibling explainer-html-docs
    # skill as an opt-in component (the card filter / page-nav styling):
    # <root>/explainer-reading-site-library-base/scripts/library.py
    #   -> <root>/explainer-html-docs/assets/components/reading-nav/reading-nav.css
    return (
        Path(__file__).resolve().parents[2]
        / "explainer-html-docs"
        / "assets"
...
def reading_nav_js_source():
    ...
    # and this library index. Owned by explainer-html-docs as an opt-in component:
    #   -> <root>/explainer-html-docs/assets/components/reading-nav/reading-nav.js
    return (
        Path(__file__).resolve().parents[2]
        / "explainer-html-docs"
        / "assets"
...
# before first paint.
INDEX_SCRIPTS = (
    "<script>try{var t=localStorage.getItem('explainer-html-docs-theme');"
...
# Document-type-neutral identity for the shared index. The deploy root is shared
# across studios (pdf-explainer, paper-explainer), so the generator/footer name carries
# no single studio's identity.
GENERATOR = "reading-library"
...
<main>
  <div class="hero">
    <p class="kicker">ライブラリ</p>
    <h1>{title}</h1>
    <p class="lede">{lede}</p>
    <div class="meta"><span class="chip">{count}件</span></div>
  </div>
{body}
</main>
...
def render_index(meta):
    ...
    if books:
        lede = "ライブラリに追加されたドキュメントの一覧。各カードから内容と音声ガイドを開けます。"
        ...
        lede = "まだドキュメントがありません。deploy-site スキルで追加してください。"
        body = "  <p>（ライブラリは空です）</p>"
    return INDEX.format(
        title=title,
...
def write_public_scaffold(meta):
    ...
    if base.exists():
        ...
        # Fallback: minimal readable stylesheet if explainer-html-docs is not
...
def main():
    ap = argparse.ArgumentParser(description="Manage a reading-library site.")
    sub = ap.add_subparsers(dest="cmd", required=True)

    # Shared across every subcommand: the caller-injected XDG namespace. It must be
    # available before any command, because root() needs it to locate library.json
    # (so it cannot be read back from the metadata). --namespace overrides the
    # LIBRARY_NAMESPACE env var; both default to "pdf-explainer" (no migration).
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument(
        "--namespace",
        default=os.environ.get("LIBRARY_NAMESPACE") or "pdf-explainer",
        help="XDG library namespace under $XDG_DATA_HOME (default: pdf-explainer)",
    )

    sub.add_parser("path", parents=[common])
    sub.add_parser("status", parents=[common])
    sub.add_parser("project", parents=[common])
    sub.add_parser("public", parents=[common])
    ...
    args = ap.parse_args()
    global LIBRARY_NAMESPACE
    LIBRARY_NAMESPACE = args.namespace
    if args.cmd == "path":
        print(root())
