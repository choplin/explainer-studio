...
`pdf-explainer-generate-site` builds its pages through the [[explainer-html-docs]] generator, passing this dir as the generator's `--context` (so `reading-site.css` is copied into each built site's `assets/`) and `--component reading-nav` (so the widget bundle is copied alongside `base.css`/`base.js`, which come from `--assets`). `library.py` copies all of them into the library's `public/assets/` for the index page — so the library index and every book page share one visual language and one interaction kit.
