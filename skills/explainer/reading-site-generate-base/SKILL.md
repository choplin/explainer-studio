---
name: reading-site-generate-base
...
# reading-site-generate-base — the shared reading-site build pipeline
...
internet is [[reading-site-deploy]]'s job — keeping the two apart lets the
...
not hand-written HTML. The [[reading-site-page]] procedure rewrites each report
for web reading (lede, key-points box, scannable sections) as Markdown + fenced divs
that name only *meaning*; the [[html-docs]] generator then binds that
...
call, guided by the `reading-site-page` rules.
...
If either `html-docs` or `reading-site-library-base` is not installed, stop
...
2. **If `<WORK_DIR>/site/` already exists, ask the user before clearing it** (no
   ...
   behind — which matters because a later [[reading-site-deploy]] run publishes
...
**2a — Author one source file per report** by applying the **[[reading-site-page]]**
...
- **Under Claude Code**, dispatch one `reading-site-page` subagent per report
  (multiple Agent calls in one message) so each page is authored in an isolated context
  and they run concurrently.
- **Otherwise**, apply the `reading-site-page` skill once per report.
...
```bash
html-docs/scripts/build-site.sh <WORK_DIR>/src <WORK_DIR>/site \
  --assets html-docs/assets \
  --context reading-site-library-base/assets \
...
root); if `html-docs` or `reading-site-library-base` is missing, stop and say
so.
...
that [[reading-site-deploy]] can put it on the internet (as a subpath of the shared
Cloudflare Pages library, Access-protected) — or offer to run it. If nothing has been
deployed before, the one-time [[reading-site-initialize]] setup must run first. Do
...
- **`site/` and `src/` are disposable and rebuilt from source.** When they already
  ...
  [[reading-site-deploy]] run publishes whatever is on disk, so prefer a clean
...
- **Restructuring is the deliverable (report pages).** A source whose heading sequence
  ...
  the instruction to restructure (the `reading-site-page` procedure states the same
  rule).
...
- [ ] The user was told the site is built under `<WORK_DIR>/site/` and pointed to
      [[reading-site-deploy]] for putting it online.
