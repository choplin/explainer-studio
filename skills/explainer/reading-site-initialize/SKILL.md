---
name: reading-site-initialize
description: This skill should be used once, before the first deploy, to set up the shared hosting target for pdf-explainer sites — a single Cloudflare Pages project (plus a local library directory under XDG) that books deploy into as subpaths, with a Cloudflare Access policy so the collection is not left public. Triggers on "サイトのセットアップをして", "配信の初期設定", "Pagesプロジェクトを用意して", "initialize the site", "set up hosting", "set up Cloudflare Pages". Should NOT trigger for deploying a book (use reading-site-deploy), building a book's site (use pdf-explainer-generate-site), or once the library already exists (deploy-site handles subsequent deploys).
user-invocable: true
---

# Initialize Site — one-time hosting setup

Set up the **single hosting target** that every pdf-explainer book is later published into. Run this **once**; afterward [[reading-site-deploy]] adds each book as a subpath to the same Cloudflare Pages project, so there is one URL and one Cloudflare Access policy for the whole collection.
...
- Not already initialized: `python3 reading-site-library-base/scripts/library.py status --namespace pdf-explainer`. If it prints existing metadata, the library exists — skip this skill and use [[reading-site-deploy]].
...
5. **First deploy (empty index) and verify:**
   ```sh
   wrangler pages deploy "$(python3 reading-site-library-base/scripts/library.py public --namespace pdf-explainer)" \
...
Report the production URL and that the library is ready for [[reading-site-deploy]] to add books.
...
- [ ] The user was told the library is initialized and that [[reading-site-deploy]] adds books to it.
