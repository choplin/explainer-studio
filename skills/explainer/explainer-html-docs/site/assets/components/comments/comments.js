...
(function () {
  ...
  var STORE_KEY = "explainer-html-docs-comments:" + docId();
  ...
  function buildPanel() {
    ...
    // A speech-bubble comment icon as inline SVG, stroked with currentColor so it
    // follows the theme like ☰ / ✕ — monochrome and vector, not the colour emoji
    // 💬. Inline SVG (not an icon font) keeps the component offline: no CDN and no
    // embedded font binary.
    fab.innerHTML =
      '<svg class="comments-fab-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" ' +
      ...
      '</svg> <span class="comments-fab-count">0</span>';
