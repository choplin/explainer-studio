...
description: "The design system and deterministic generator for any skill that produces self-contained explainer HTML. Consumer skills provide semantic Markdown with meaning-only fenced divs; this skill validates the vocabulary, rejects unknown variants and classes, wraps tables, emits static HTML, and supplies shared assets, components, and progressive enhancements. Consumers may layer context-specific CSS and opt-in components, but do not hand-author mechanical markup. Use when implementing or running an explainer that needs this HTML contract."
---
