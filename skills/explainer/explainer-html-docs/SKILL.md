...
a browsable web document** — [[explainer-diff]] (a diff explained to a
...
- **Inline mode** (a single self-contained file): `base.css`/`base.js` are folded into
  ...
  [[explainer-diff]] takes this route.)
...
site-name: explainer-html-docs   # header.site link text
...
or [[explainer-diff]]'s risk / verify axes) supplies:
...
`<out-dir>/assets/`, so a copy-mode site consumes an explainer-html-docs-owned component the
...
- **The semantic half** — is this the right variant, is this actually the takeaway,
  ...
  **internalizes it as its own completion check** — e.g. [[explainer-diff]]
...
`explainer-html-docs` review step to run. A consumer that layers high-stakes
...
reading page and on an [[explainer-diff]] page.
...
- **`base.js` targets the first `main article`.** A page that puts several
  ...
  [[explainer-diff]] takes that route (its own template variant). This is
...
- **Nothing here fails loudly at the *semantic* level.** The generator refuses a
  ...
  [[explainer-diff]] does for `risk`/`tested`/`verified`).
