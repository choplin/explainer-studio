...
# explainer-reading-site-generate-base — the shared reading-site build pipeline

This skill owns the **workflow shared by every skill that turns a work dir's
`reports/` into a reading-guide website** — today [[pdf-explainer-generate-site]]
(book chapters) and [[paper-explainer-generate-site]] (paper perspectives), and any
...
internet is [[explainer-reading-site-deploy]]'s job — keeping the two apart lets the
...
- **pdf-explainer**: `overview` → kicker `全体レポート` first, then `chapter-N` in
...
- **paper-explainer**: a fixed perspective table — `overview`→`全体レポート`,
...
[[explainer-audio-dialogue]] → [[explainer-audio-narrate]] first.

The pages are **authored, not converted** — and authored as **semantic Markdown**,
not hand-written HTML. The [[explainer-reading-site-page]] procedure rewrites each report
for web reading (lede, key-points box, scannable sections) as Markdown + fenced divs
that name only *meaning*; the [[explainer-html-docs]] generator then binds that
...
call, guided by the `explainer-reading-site-page` rules.
...
- **base substrate** (`base.css` / `base.js`: reading-progress bar, table of contents
  ...
  are owned by **[[explainer-html-docs]]** — consumed as the generator's
...
- the **content context layer** (`reading-site.css` — content styling only) is owned by
  **[[explainer-reading-site-library-base]]**, passed to the generator as `--context` (the deploy
  library index uses the same layers).

If either `explainer-html-docs` or `explainer-reading-site-library-base` is not installed, stop
...
2. **If `<WORK_DIR>/site/` already exists, ask the user before clearing it** (no
   ...
   behind — which matters because a later [[explainer-reading-site-deploy]] run publishes
...
**2a — Author one source file per report** by applying the **[[explainer-reading-site-page]]**
...
- **Under Claude Code**, dispatch one `explainer-reading-site-page` subagent per report
  (multiple Agent calls in one message) so each page is authored in an isolated context
  and they run concurrently.
- **Otherwise**, apply the `explainer-reading-site-page` skill once per report.
...
rewrites: navigation is rendered at runtime by `reading-nav.js` (the explainer-html-docs
...
root); if `explainer-html-docs` or `explainer-reading-site-library-base` is missing, stop and say
so.
...
that [[explainer-reading-site-deploy]] can put it on the internet (as a subpath of the shared
Cloudflare Pages library, Access-protected) — or offer to run it. If nothing has been
deployed before, the one-time [[explainer-reading-site-initialize]] setup must run first. Do
...
- **`site/` and `src/` are disposable and rebuilt from source.** When they already
  ...
  [[explainer-reading-site-deploy]] run publishes whatever is on disk, so prefer a clean
...
- **Restructuring is the deliverable (report pages).** A source whose heading sequence
  ...
  the instruction to restructure (the `explainer-reading-site-page` procedure states the same
  rule).
...
- **Reuse, don't fork, the shared assets.** The content context layer (`reading-site.css`)
  comes from `explainer-reading-site-library-base` (the name is the shared context layer, not a
  pdf-explainer-only dependency) and is passed as `--context`; the reading-site nav widgets
  come from explainer-html-docs' `reading-nav` component via `--component reading-nav`.
...
- [ ] The user was told the site is built under `<WORK_DIR>/site/` and pointed to
      [[explainer-reading-site-deploy]] for putting it online.
